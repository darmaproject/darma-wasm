...
import (
	...
	"github.com/darmaproject/darma-wasm/exec/internal/compile"
...
type Interpreter struct {
	*VM
	Memory           *darma.WavmMemory
	heapPointerIndex int64
	Mutable          *bool
}

func NewInterpreter(module *wasm.Module, compiled []darma.Compiled, initMem func(m *darma.WavmMemory, module *wasm.Module) error, captureOp func(pc uint64, op byte) error, captureEnvFunctionStart func(pc uint64, name string) error, captureEnvFunctionEnd func(pc uint64, name string) error, debug bool) (*Interpreter, error) {
	...
	vm.debug = debug
	inter.Memory = darma.NewWavmMemory()
...
func NewWavmProcess(vm *VM, memory *darma.WavmMemory, mutable *bool) *WavmProcess {
	return &WavmProcess{
		vm,
		memory,
		mutable,
